...
public class PlayerMgr : MonoBehaviour
{
    ...
    [Tooltip("プレイヤーのメッシュ")][SerializeField] List<GameObject> playerMeshes;
        ...
        if (stopTimer == 0)
        {
            ...
            if(blinkTimer != 0)
            {
                ...
                if(blinkTimer <= blinkNextTime)
                {
                    ...
                    if(hitter.GetComponentInParent<Player>().GetPlayerNumber() == 1)
                    {
                        if (playerMeshes[0].activeSelf)
                        {
